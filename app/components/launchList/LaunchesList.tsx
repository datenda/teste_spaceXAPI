...
export default function LaunchesTable() {
  ...
  const handleRowClick = (launch: Launch) => {
    ...
    const updated = [launch, ...recentLaunches.filter((l) => l.id !== launch.id)].slice(0, 5);
  ...
  return (
    <div ref={containerRef} className="max-h-[800px] overflow-y-auto bg-black p-4 rounded-lg">
  ...
  <Table className="w-full table-auto">
    ...
    <TableBody>
      {launches.map((launch) => (
        ...
        >
          <TableCell className={cellClasses}>{launch.mission_name}</TableCell>
          <TableCell className={cellClasses}>{launch.rocket.rocket_name}</TableCell>
          <TableCell className={cellClasses}>{launch.details ?? "N/A"}</TableCell>
